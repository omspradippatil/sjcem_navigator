...
const LIVE_HEARTBEAT_SYNC_INTERVAL_MS = 1000;
...
function startLiveHeartbeatSync() {
  ...
  }, LIVE_HEARTBEAT_SYNC_INTERVAL_MS);
}
...
async function applyOcrTimetableFromText(rawText) {
  ...
  try {
    ...
    let subjectsQuery = state.supabase
      .from("subjects")
      .select("id,name,code,branch_id,semester,is_lab")
      .eq("branch_id", branchId);
    subjectsQuery = applySelectScope(subjectsQuery, subjectModule);
    const { data: subjectsInBranch, error: subjectLoadError } = await subjectsQuery;
    if (subjectLoadError) {
    ...
    if (missingSubjects.length) {
      let insertSubjectsQuery = state.supabase.from("subjects").insert(missingSubjects);
      insertSubjectsQuery = applyWriteScope(insertSubjectsQuery, subjectModule);
      const { error: insertSubjectsError } = await insertSubjectsQuery;
      if (insertSubjectsError) {
        throw insertSubjectsError;
      }

      let refreshedSubjectsQuery = state.supabase
        .from("subjects")
        .select("id,name,code,branch_id,semester,is_lab")
        .eq("branch_id", branchId);
      refreshedSubjectsQuery = applySelectScope(refreshedSubjectsQuery, subjectModule);
      const { data: refreshedSubjects, error: refreshSubjectsError } = await refreshedSubjectsQuery;
    ...
    if (subjectIds.length) {
      let mappingQuery = state.supabase
        .from("teacher_subjects")
        .select("teacher_id,subject_id")
        .in("subject_id", subjectIds);
      mappingQuery = applySelectScope(mappingQuery, teacherSubjectsModule);
    ...
    let activeTeachersQuery = state.supabase
      .from("teachers")
      .select("id,branch_id,is_active")
      .eq("branch_id", branchId)
      .eq("is_active", true);
    activeTeachersQuery = applySelectScope(activeTeachersQuery, teacherModule);
    const { data: activeTeachers, error: activeTeachersError } = await activeTeachersQuery;
    ...
    if (subjectIds.length) {
      let historicalQuery = state.supabase
        .from("timetable")
        ...
        .in("subject_id", subjectIds);
      historicalQuery = applySelectScope(historicalQuery, timetableModule);
      const { data: historicalRows, error: historicalError } = await historicalQuery;
      if (historicalError) {
    ...
    let existingSlotsQuery = state.supabase
      .from("timetable")
      .select("id,day_of_week,period_number,batch")
      .eq("branch_id", branchId)
      .eq("semester", targetSemester)
      .in("day_of_week", daySet);
    existingSlotsQuery = applySelectScope(existingSlotsQuery, timetableModule);
    const { data: existingSlots, error: existingSlotsError } = await existingSlotsQuery;
    ...
    for (const payload of rows) {
      ...
      if (existingId) {
        ...
        let updateQuery = state.supabase.from("timetable").update(payload).eq("id", existingId);
        updateQuery = applyWriteScope(updateQuery, timetableModule);
        ...
        let insertQuery = state.supabase.from("timetable").insert(payload);
        insertQuery = applyWriteScope(insertQuery, timetableModule);
    ...
    if (missingPairs.length) {
      let mapInsertQuery = state.supabase
        .from("teacher_subjects")
        .upsert(missingPairs, { onConflict: "teacher_id,subject_id" });
      mapInsertQuery = applyWriteScope(mapInsertQuery, teacherSubjectsModule);
      const { error } = await mapInsertQuery;
      if (error) {
        throw error;
      }
    }
  ...
  try {
    ...
    if (updateExisting && codes.length) {
      let existingQuery = state.supabase
        ...
        .eq("branch_id", branchId);
      existingQuery = applySelectScope(existingQuery, subjectModule);
    ...
    for (const item of subjects) {
      ...
      if (existing) {
        let updateQuery = state.supabase
          ...
          .update(payload)
          .eq("id", existing.id);
        updateQuery = applyWriteScope(updateQuery, subjectModule);
        const { error } = await updateQuery;
        if (error) {
          throw error;
        }
        ...
        let insertQuery = state.supabase.from("subjects").insert(payload);
        insertQuery = applyWriteScope(insertQuery, subjectModule);
        const { error } = await insertQuery;
        if (error) {
          throw error;
        }
